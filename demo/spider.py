...
class Spider(threading.Thread):
    ...
    def get_Page(self, page, proxies, url_manager):
        ...
        try:
            ...
            # 页面上可能有两个地址，取最后一个匹配的地址
            jsfile_url = "http://" + model[len(model) - 1]
        except Exception as e:
            print(e)
            jsfile = requests.get(jsfile_url, headers=headers, proxies=proxies, timeout=3).text

            constant = e.search(r'.*remove\(\);var\sc=\w+\(e,\"(\w+)\".*', jsfile).group(1)
            '''
    ...
    def run(self):
        for page in self.pages:
            self.get_Page(page, self.proxies, self.url_manage)
